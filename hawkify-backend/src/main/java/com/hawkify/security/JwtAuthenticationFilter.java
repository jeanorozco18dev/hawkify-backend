...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (jwtService.esTokenValido(token) && SecurityContextHolder.getContext().getAuthentication() == null) {
            UUID usuarioId = UUID.fromString(jwtService.extraerUsuarioId(token));

            usuarioRepository.findByIdConRol(usuarioId).ifPresent(usuario -> {
                if (usuario.getEstado() != EstadoUsuario.ACTIVO) {
                    return;
                }

                var authority = new SimpleGrantedAuthority("ROLE_" + usuario.getRol().getNombre().toUpperCase());
                var authToken = new UsernamePasswordAuthenticationToken(usuario, null, List.of(authority));
